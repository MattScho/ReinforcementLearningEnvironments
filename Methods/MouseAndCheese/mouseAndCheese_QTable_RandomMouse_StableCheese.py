...
class MaC_QTable:
    ...
    def __init__(self):
        ...
        self.QTable = np.random.rand(10,10,4)
    ...
    def act(self, state):
        ...
        pred = self.QTable[state[0]][state[1]]
    ...
    def replay(self, curState, action, reward, newState):
        ...
        :return:
        '''
        self.QTable[curState[0]][curState[1]][action] = self.QTable[curState[0]][curState[1]][action]\
            + self.learningRate * (reward + self.gamma * np.max(self.QTable[newState[0]][newState[1]]) -\
                                   self.QTable[curState[0]][curState[1]][action]
                                   )
...
for trial in range(trials):
    ...
    curState = [env.getMousePosition()[0], env.getMousePosition()[1]]
    # Run trial
    for step in range(trial_len):
        ...
        if done:
            reward = 10
        # Get new state
        newState = [env.getMousePosition()[0], env.getMousePosition()[1]]
